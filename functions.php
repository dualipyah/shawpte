...
function jquery_scripts() {
	...
	//bootstrap
//	wp_enqueue_style( 'bootstrapcss', get_template_directory_uri() . '/css/bootstrap.min.css',true,'4.3.1','all');
//	wp_enqueue_script( 'bootstrapjs', get_template_directory_uri() . '/js/bootstrap.min.jss', array ( 'bootstrap' ), 1.1, true);
}
...
function my_theme_scripts_function() {
  wp_enqueue_script( 'myscript', get_template_directory_uri() . '/js/contact-us.js',array ( 'jquery' ), 3.4, true);
}
...
function getRefererPage( $form_tag ){
	if ( $form_tag['name'] == 'referer-page' ) {
		$name = "DEMOGRAPHICS_REFERER";
		$value = $_SERVER["HTTP_REFERER"];
		setcookie($name,$value,time() + 7200, "/"); // set cookie expiration to 2 hrs
		$form_tag['values'][] = $_COOKIE["DEMOGRAPHICS_REFERER"];
		if(!isset($_COOKIE[$name])){
			$form_tag['values'][] = htmlspecialchars($_SERVER["HTTP_REFERER"]);
		}else{
			$form_tag['values'][] = htmlspecialchars($_COOKIE["DEMOGRAPHICS_REFERER"]);
		}
